...
- (void)beginLoginWithWithStyle:(PNSBuildModelStyle)style {
    ...
    [[TXCommonHandler sharedInstance] getLoginTokenWithTimeout:3.0
        ...
        if ([PNSCodeLoginControllerPresentSuccess isEqualToString:resultCode]) {
            ...
            NSLog(@"页面已完全销毁dealloc:%@", resultDic);
...
- (NSArray<NSDictionary<NSString *,id> *> *)dataSource {
    if (!_dataSource) {
        _dataSource = @[
            ...
            @{
                @"section_title" : @"二次协议确认弹窗",
                @"data" : @[
                    @{
                        @"title" : @"二次协议弹窗竖屏（弹窗）",
                        @"style" : @(PNSBuildModelStylePrivacyAlertPortraitAlert)
                    },
                    @{
                        @"title" : @"二次协议弹窗竖屏（操作表）",
                        @"style" : @(PNSBuildModelStylePrivacyAlertPortraitActionSheet)
                    },
                    @{
                        @"title" : @"二次协议弹窗横屏（弹窗）",
                        @"style" : @(PNSBuildModelStylePrivacyAlertLandscapeAlert)
                    },
                    @{
                        @"title" : @"二次协议弹窗横屏（操作表）",
                        @"style" : @(PNSBuildModelStylePrivacyAlertLandscapeActionSheet)
                    },
                    @{
                        @"title" : @"弹窗授权页二次协议弹窗",
                        @"style" : @(PNSBuildModelStyleAlertPrivacyAlert)
                    }
                ]
            },
            @{
